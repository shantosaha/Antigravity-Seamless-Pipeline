...
2. **Read Pipeline Output** — Check the pre-pipeline results:
   ...
   - **Layer 7 (Skill):** Experience API checked first! If matched pattern exists with >80% confidence, it overrides static TF-IDF semantic matching. 
...
4. **Understand Requirements** (Node: `understand_requirements`)
   ...
   - **Agent 15 (Orchestrator) Check:** Determine if the task needs to be routed to specific specialist agents based on the Experience API recommendation.
...
9. **Report Results** — Include in your final response:
   ...
   - **Confirm that the Experience API recorded the task outcome for future agent learning.**
